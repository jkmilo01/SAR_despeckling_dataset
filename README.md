...
5. The ground truth image will be the same size as the reference image, by averaging the images obtained in the previous step (Use 3. Generate_GTruth.py). 
6. Image clip is performed by firs defining three parameters: height, width and step. The default is height=width=step=512. The images (noisy and ground truth) will be loaded and then cropped in two folders named 'Gtruth' and 'Noisy'. Depending of the setting of the parameters, the resulting dataset will be bigger or smaller. These two folders can be used for training deep learning models. (Use 4. Clip_Image.py)
